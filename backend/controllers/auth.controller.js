...
import { generateTokenAndSetCookies, generateVerificationToken } from "../utils/generateVerificationToken.js";
import { sendVerificationToken, sendWelcomeEmail , sendPasswordResetEmail ,sendResetSuccessEmail } from "../mailtrap/emails.js";
...
export const signUp=async(req ,res)=>{
    ...
    try {
     ...
     const user= await User.create({
        email , name , password:hashedPassword , verificationToken,
...
export const login=async(req ,res)=>{
  ...
  try {
     ...
     if(!isPasswordCorrect ||!user)
     {
        return res.status(404).json({
            success:false ,
            message:"Invalid credentials please try with corect credentials"
        })
     }
